...
def link_1(db_name):  # 数据库文件名称
    ...
    print("2.bigen into memery. Datetime: " + time.strftime('%Y-%m-%d %H:%M:%S',
    ...
    cursor.execute(
        "create table link_1(l1_id integer primary key,l2_id,per_off,tail_off,offset_1,offset_2,page_sum,page_no_1,page_no_2,file_no_1)")
    cursor.execute("create index link1_idx on link_1(offset_1,page_no_1,file_no_1)")  # 创建索引
    print(
        "4.bigen insert. Datetime: " + time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time())))  # current time
