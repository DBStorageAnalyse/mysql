...
class Scan_1(QThread):   # 类Scan_1继承自 QThread
    ...
    def run(self):
        self.innodb_scan(self.f_name,self.db_name,self.logging,self.start_off,self.end_off,self.endian,self.page_size,self.scan_size)
    ...
    # f_name是要扫描的源文件，start_off扫描的起始偏移，[chk_level是校验等级]，db_name输出的db名
       # logging.basicConfig(level=logging.DEBUG,format='%(message)s',filename='%s_%s.log'%(db_name[:-3],time.strftime('%Y%m%d%H%M%S',time.localtime(time.time()))),filemode='w')
        ...
        f = open (f_name,'rb')
        f1 = open (db_name,'w'); f1.close() # 数据库文件
        conn = sqlite3.connect(db_name)
        cursor = conn.cursor()
        cursor.execute("create table innodb_page(id integer primary key,offset,ts_no,page_no,page_type,pre_page,next_page,index_level,index_id,rec_sum)")  # 大表，用于小库
      #  cursor.execute("create table link_1(l1_id integer primary key,l2_id,per_off,tail_off,offset_1,offset_2,page_sum,page_no_1,page_no_2,file_no_1)")
        ...
        sum = 0
        page_0 = 0; page_8 = 0; page_17855 = 0
        ...
        for i in range(0,loop_1+1):         # 处理文件的所有buffer块
                    ...
                    if chk2 == 1 :
                        ...
                        if chk3 == 1 :
                            ...
                            if chk1 == 1 :
                                ...
                                values(?,?,?,?,?,?,?,?,?)",(offset,data2[9],data2[1],data2[7],data2[2],data2[3],index_level,index_id,rec_sum))
                                if sum%10000 == 0 :
            ...
            if i%(loop_1//1000+1) == 0 or i==loop_1:    # 输出扫描进度
                ...
                print("Buf:%d/%d,Percent:%4.1f%%, Find:%d=%dM, I/O:%4.1fM/s,Time:%dMin\r"%(i,loop_1,progress,sum,sum*page_size/1024/1024,speed,(loop_1+1-i)*8/((speed+0.01)*60)),end="")
        ...
        print(" ,Used: %d:"%((end-begin)//3600) + time.strftime('%M:%S',time.localtime(end-begin)),end='') # 用时
