...
def parse_len(type_name, column_stmt):
    type_len = {'text': 0, 'mediumtext': 0, 'longtext': 0, 'date': 3, 'datetime': 5, 'timestamp': 4, \
                'tinyint':1,'smallint': 2, 'mediumint': 3, 'int': 4, 'bigint': 8, 'float': 4, 'double': 8, 'enum': 4,'bit':1 }
...
def parse_create_stmt(create_stmt):
  ...
  #  iter = re.finditer(r'\n`(\w+)`  (\w+).*', create_stmt)  #　可改, Navicat
    ...
    for m in iter:   # 每一行
        ...
        column.var_len_is = int(m.group(2) in ('char','varchar','text','mediumtext','longtext','tinytext','longtblob','blob','mediumblob','tinyblob'))    # 变长类型
...
def save_table2(table,out_db):
    ...
    for col in table.col:
        cursor.execute("insert into col_info(tab_id,col_id,col_name,col_type,col_len,prec,scale,notnull_is,pkey_is,var_len_is,unsign_is) values(%d,%d,'%s','%s',%d,%d,%d,%d,%d,%d,%d)"%\
        (table.num,col.col_id,col.column_name,col.data_type,col.column_len,col.prec,col.scale,col.notnull_is,col.pkey_is,col.var_len_is,col.col_unsign))
    conn.commit()
...
def table_frm(sql_file_name,out_db):
    ...
    data = f.read()
    f1 = open(out_db,'wb'); f1.close()
    conn = sqlite3.connect(out_db)  # 打开数据库
    cursor = conn.cursor()
    cursor.execute("create table tab_info(id integer primary key,db_name,tab_name,tab_type,tab_id,ind_id,ts_no,col_sum,nullable_sum,var_len_sum)")
    cursor.execute("create table col_info(id integer primary key,tab_id,col_id,col_name,col_type,col_len,prec,scale,notnull_is,pkey_is,var_len_is,unsign_is)")
