...
def rec_compact(f,in_data,table):   #解析 compact 记录  in_data(输入页面数据),table(表结构)
    ...
    if page1.page_level != 0 : # 聚集索引的 索引结点，记录结构不同，是索引记录
        page_1 = page1
        in_data1 = in_data
        print('talbe:%s,level:%s,page_no:%d,rec_sum:%d'%(table.tab_name,page_1.page_level,page_1.page_no,page_1.rec_sum1))
        for i in range(0,page_1.rec_sum1+2):  # 解记录
            ...
            col_off = 0; var_list = 0   # 变长列长度列表的长度
            ...
            for ii3 in range(len(table.index_col)):
                ...
                try:
                    ...
                    print('UnboundLocalError:pg_no:%d,crd_no:%d,off:%d,col_no:%d,len:%d '%(page1.page_no,i,off_set,ii3,len1))
                    col_data2 = ''
                ...
                if table.col_1[ii3].notnull_is == 0:        # 此列可为 null
                    ...
                    if col_is_null == 0 :  # 列不为空
                        if table.col_1[ii3].varlen_is == 0 :    # 是定长
                            ...
                            var_list = var_list + 1
                            var_len1 = s("B",in_data[off_set-null_map_len-5-var_list:off_set-null_map_len-5-var_list+1])
                            len1 = var_len1[0]
                            if var_len1[0] >= 128 :
                                var_list = var_list + 1
                                var_len1 = s("BB",in_data[off_set-null_map_len-5-var_list:off_set-null_map_len-5-var_list+2])
                                len1 = (var_len1[1]-128)*256 + var_len1[0]
                                if len1 >= 16384 :   # 行溢出
                                    yichu_is = 1
                                    len1 = len1 - 16384
                                    ...
                                    for i in range(10) :
                                        ...
                                        next_len1 = next_1[0]; next_page = next_1[1]
                        ...
                        try:
                            ...
                            print('struct.error:pg_no:%d,crd_no:%d,off:%d,col_no:%d,len:%d '%(page1.page_no,i,off_set,ii3,len1))
                            col_data2 = ''
            ...
            for i1 in range(table.col_sum):       # 调整列顺序
                for i2 in range(len(table.col_1)):
                    if table.col[i1].col_id == table.col_1[i2].col_id:
                        try:
                            record1.col_data1.append(record1.col_data2[i2])   #  col_data1 是最终解析好的，可直接输出的，跟表结构相同。col_data2是记录存储中的列顺序。
                        except IndexError:
    ...
    if page1.page_level != 0 : # 聚集索引的 索引结点，记录结构不同
        ...
        for i in range(0,page_1.rec_sum1+2):   # 解记录
            ...
            for ii3 in range(col_sum_1):
                ...
                if var_len1[0] >= 128 :
                    var_list = var_list + 2
                    var_len1 = s("B",in_data1[off_set - 6 - var_list+1:off_set - 6 - var_list+2])  # ================
            ...
            if table.tab_obj_id == 1:
                print('i:%d,off_set_now:%d,off_set_next:%d,table_name:%s,page_no:%d'%(i,off_set_now,off_set,table_name,page_no_1))
            f.seek(page_no_1*16384)
            in_data = f.read(16384)
        ...
        for i in range(0,page1.rec_sum1+2):   # 解记录
            ...
            #    len1 = table.col_1[ii3].col_len
                ...
                if var_len1[0] >= 128 :
                    var_list = var_list + 2
                    var_len1 = s("B",in_data[off_set - 6 - var_list+1:off_set - 6 - var_list+2])  # ================
                len1 = var_len1[0] - col_off
                col_off = var_len1[0]
            ...
            if page1.page_no == 868382 :
                print('i:%d,off_set_now:%d,off_set_next:%d,table_name:%s,page_no:%d'%(i,off_set_now,off_set,'',page1.page_no))
        return page1
