...
class Unload_DB():
    ...
    def unload_db(self,file_infos,db):
        ...
        sys_tables,sys_columns,sys_indexes,sys_fields = init.sys_tables(f,sys_tables_page,sys_columns_page,sys_indexes_page,sys_fields_page)
    ...
    def tab_info(self,tab,db):
        ...
        for table in tab:
            cursor.execute("insert into tab_info(db_name,tab_name,tab_type,tab_id,ind_id,col_sum) values('%s','%s',%d,%d,%d,%d)"% \
            ...
            for col in table.col:
                if col.tab_obj_id == table.tab_obj_id:
                    ...
                    (table.tab_obj_id,col.col_id,col.col_name,col.col_type,col.col_len,col.prec,col.scale,col.notnull_is,col.pkey_is,col.varlen_is,col.col_unsign))
         #   conn.commit()
        conn.commit()
