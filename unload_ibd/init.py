...
# 获取文件头信息
def first_page(data):
    file_info = table_struct.File_Info(); fmt_1 = '>'
...
def init_all(f,sys_tables,sys_columns,sys_indexes,sys_fields):
    ...
    for i1 in range(len(sys_tables)):
        for ii1 in range(len(sys_tables[i1].record)):
            ...
            tab_all.append(tab)
            print('tab_id:%d,tab_name:%s.%s,col_sum:%d,index_id:%d,ts_no:%d,pgfirst:%d'%(tab.tab_obj_id,db_name,tab.tab_name,tab.col_sum,tab.index_id,tab.ts_no,tab.pgfirst))
...
def tab_info(sys_columns,sys_indexes,sys_fields,obj_id,tab_name):
    ...
    for i2 in range(len(sys_indexes)):        #　获取索引信息，主键信息
        for ii2 in range(len(sys_indexes[i2].record)):
            if sys_indexes[i2].record[ii2].col_data1[0] == obj_id and sys_indexes[i2].record[ii2].col_data1[4] in(1,3):
        ...
        for ii in range(len(sys_columns[i].record)):
            if sys_columns[i].record[ii].col_data1[0] == obj_id:
                ...
                if obj_id == 19 :                    # 测试用，显示一个表的列信息,obj_id = tab_id
                    print(column1.col_id,column1.col_name,column1.col_type,'len:%d,mtype:%d,ptype:%d,p:%d'%(column1.col_len,col_mtype,col_ptype_1,col_ptype))
    ...
    for ii in range(0,table1.col_sum):
        ...
        else: ss += '%s,'%table1.col[ii].col_name
    table1.sql = ss     #  s1 + ss + ');\n'
    return table1
        ...
        for i in range(0,len(tab_data)):
            for i1 in range(0,tab_data[i].rec_sum):
                ...
                for i2 in range(0,tab.col_sum):
                    ...
                    else: ss += '\''+str(tab_data[i].record[i1].col_data1[i2])+'\'' + ','
                print(ss)
    ...
    for i in range(len(tab_data)):
        for i1 in range(len(tab_data[i].record)):
            ...
            for i2 in range(tab.col_sum):       # 处理每一列
                if i2 == tab.col_sum-1 :
                    try:
                        if tab.col[i2].col_type in('char','varchar','text','date','datetime','timestamp','blob') and tab_data[i].record[i1].col_data1[i2] != 'NULL':
